...
with tab1:
    ...
    if audio_data_1:
        ...
        try:
            ...
            with res_c2: st.markdown(f"<div class='report-box'><p style='margin:0; font-size:0.7rem; color:#AAA;'>음역대</p><h4 style='margin:0;'>{range_type}</h4></div>", unsafe_allow_html=True)
            with res_c3: st.markdown(f"<div class='report-box'><p style='margin:0; font-size:0.7rem; color:#AAA;'>평균 주파수</p><h4 style='margin:0;'>{avg_f0:.1f}Hz</h4></div>", unsafe_allow_html=True)

            col_graph, col_ai = st.columns([1.1, 0.9], gap="small")
            with col_graph:
                fig1, (ax1, ax2) = plt.subplots(2, 1, figsize=(6, 3.5), gridspec_kw={'height_ratios': [1, 1.2]})
            ...
            with col_ai:
                ...
                st.markdown("<h4 style='margin-bottom: 5px;'>📊 Gemini AI 보컬 리포트</h4>", unsafe_allow_html=True)
                with st.spinner('🤖 AI 분석 중...'):
                    model = genai.GenerativeModel("gemini-3.1-flash-lite")
                    sample_file = genai.upload_file(path=tmp_path)
                    # [AI 중복 및 형식 수정] 중복 방지 및 고정 형식Prompt
                    prompt = f"""
                    데이터: {avg_f0:.1f}Hz, {gender_type}.
                    당신은 전문 보컬 트레이너입니다. 다음 형식을 '엄격히' 지켜서 매우 짧게 작성하세요:
                    1. 판정 이유: (왜 이 성별/주파수인지 데이터 기반 설명)
                    2. 닮은 동물: (음색과 어울리는 동물)
                    3. 추천 가수: (이 목소리와 장점이 비슷한 '국내' 가수 1명. **중요: 아이유, 김동률은 제외하고 매번 다른 가수를 장르 다양하게 추천할 것**)
                    """
...
with tab2:
    ...
    # [게임 오차범위 수정] ±20Hz로 변경
    with col_ctrl: st.caption("💡 목소리로 타겟 주파수를 맞추세요! (허용 오차: ±20Hz)")
    with col_btn:
        if st.button("🔄 변경", use_container_width=True, key="reset_game"):
    ...
    if audio_data_2:
        ...
        try:
            ...
            if not np.isnan(avg_f0):
                ...
                with res_col2:
                    # [게임 승리 조건 수정] 오차범위 20Hz
                    if diff <= 20:
                        st.balloons()
                        st.markdown(f"<h2 style='color: #00CC66; margin-top: 10px; text-align: center;'>🎊 성공!</h2>", unsafe_allow_html=True)
                    else:
                        st.markdown(f"<h2 style='color: #FF4B4B; margin-top: 10px; text-align: center;'>❌ 실패</h2>", unsafe_allow_html=True)
                
                # [그래프 잘림 수정] figsize 세로 크기를 1.2로 대폭 다이어트
                fig2, ax = plt.subplots(figsize=(6, 1.2)) 
                n_fft = 1024; D = np.abs(librosa.stft(y, n_fft=n_fft)); avg_D = np.mean(D, axis=1)
                freqs = librosa.fft_frequencies(sr=sr, n_fft=n_fft)
